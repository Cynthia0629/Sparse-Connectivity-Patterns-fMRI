function C_upd = quad_estimate_C_old_norm(B_upd,lambda_2,corr)

C_upd = zeros(size(B_upd,2),size(corr,1));
for m = 1:size(corr,1)
   
    H = 2*((B_upd'*B_upd).^2/size(corr,1) + lambda_2* eye(size(B_upd'*B_upd)));
    Corr_mat = reshape(corr(m,:,:),[size(corr,2),size(corr,3)]);
    %B_hat_mat = reshape(B_hat(m,:,:),[size(corr,2),size(corr,3)]);
    M = -2*(B_upd'*(Corr_mat)*B_upd)/size(corr,1);
    f = diag(M) ;
    A = -eye(size(C_upd,1));
